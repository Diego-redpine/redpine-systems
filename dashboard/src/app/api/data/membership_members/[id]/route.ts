...
const { handleGetById, handlePut, handleDelete } = createCrudHandlers<MembershipMember>('membership_members', {
...
// PATCH - Move member to different pipeline stage
export async function PATCH(
  ...
  try {
    ...
    if (error) {
      console.error('PATCH membership_members error:', error);
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      data,
      message: `Member moved to stage ${stage_id}`,
    });
  } catch (error) {
    console.error('PATCH membership_members error:', error);
    return NextResponse.json(
