...
interface FormTemplateLibraryProps {
  colors: { buttons?: string; text?: string; headings?: string; borders?: string; cards?: string; icons?: string };
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div
          className="w-full max-w-lg max-h-[85vh] flex flex-col shadow-lg overflow-hidden"
        ...
        >
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm mt-0.5" style={{ color: colors.icons || '#6B7280' }}>
                {template.description}
              </p>
              <div className="flex items-center gap-2 mt-2">
                <DocTypeBadge docType={template.docType} size="sm" />
                <span className="text-xs" style={{ color: colors.icons || '#9CA3AF' }}>
            ...
            <button
              onClick={onClose}
              className="p-1 hover:bg-black/5 transition-colors shrink-0"
              aria-label="Close preview"
            >
              <svg className="w-5 h-5" style={{ color: colors.icons || '#9CA3AF' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="space-y-4">
              {template.fields.map((field) => (
                <div key={field.id}>
                  ...
                  {field.type === 'textarea' ? (
                    <div
                      className="w-full h-16 px-3 py-2 text-sm"
                      style={{ backgroundColor: `${borderColor}30`, border: `1px solid ${borderColor}`, color: colors.icons || '#9CA3AF' }}
                    >
                    ...
                    <div
                      className="w-full h-20 flex items-center justify-center text-sm"
                      style={{ backgroundColor: `${borderColor}20`, border: `1px dashed ${borderColor}`, color: colors.icons || '#9CA3AF' }}
                    >
                      Signature field
                    </div>
                  ) : (
    ...
    <div className="space-y-5">
      ...
      <div className="relative">
        <svg
          ...
          style={{ color: colors.icons || '#9CA3AF' }}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
        ...
        {searchQuery && (
          ...
          >
            <svg className="w-4 h-4" style={{ color: colors.icons || '#9CA3AF' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        )}
      ...
      {filteredTemplates.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-sm" style={{ color: colors.icons || '#9CA3AF' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredTemplates.map((template) => (
            <button
              ...
              className="text-left bg-white p-5 transition-all hover:shadow-md hover:-translate-y-0.5 group"
              style={{
            ...
            >
              ...
              <p
                ...
                style={{ color: colors.icons || '#6B7280' }}
              >
                {template.description}
              </p>
