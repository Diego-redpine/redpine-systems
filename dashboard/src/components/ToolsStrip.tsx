...
export default function ToolsStrip({ onOpenChat, isChatOpen, onNavigateToComms, isCommsActive, side: controlledSide, onSideChange, buttonColor, onNavigateToBrand, isBrandActive, onNavigateToSite, isSiteActive, onNavigateToMarketplace, isMarketplaceActive, onNavigateToMarketing, isMarketingActive, disableDrag }: ToolsStripProps) {
  ...
  // Auto-scroll ref for edge-zone scrolling during drag
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    // Compute per-frame delta BEFORE updating lastY
    const frameDeltaY = e.clientY - dragRef.current.lastY;

    dragRef.current.lastX = e.clientX;
    dragRef.current.lastY = e.clientY;

    // Direct DOM manipulation — bypasses React for zero-lag positioning
    // Clamp so toolbar stays below header (56px) and above viewport bottom
    const elHeight = containerRef.current.getBoundingClientRect().height;
    const halfH = elHeight / 2;
    const headerHeight = 56;
    const minY = headerHeight + halfH - window.innerHeight / 2;
    const maxY = window.innerHeight / 2 - halfH;
    const newYOffset = Math.max(minY, Math.min(maxY, dragRef.current.startOffset + deltaY));
    ...
    // Scroll the page content 1:1 with the drag movement
    const scrollContainer = document.querySelector('.overflow-auto[class*="flex-1"]') as HTMLElement | null;
    if (scrollContainer && scrollContainer.scrollHeight > scrollContainer.clientHeight && dragRef.current.moved) {
      scrollContainer.scrollTop += frameDeltaY;
    }
  }, []);
  ...
  const handlePointerUp = useCallback(() => {
    ...
    const finalYOffset = Math.max(minY, Math.min(maxY, dragRef.current.startOffset + deltaY));
  ...
  return (
    ...
    >
      <div
        ...
        className="relative flex flex-col items-center py-5 px-1.5 gap-5 border-2 select-none"
        style={{
          backgroundColor: bg,
          borderColor: bg,
        ...
        {buttons.map(btn => (
          <div key={btn.id} className="relative">
            ...
            {tooltip === btn.id && !isDragging && (
              <div
                className="absolute top-1/2 -translate-y-1/2 px-2.5 py-1.5 text-xs font-semibold border-2 whitespace-nowrap pointer-events-none"
                style={{
                  backgroundColor: bg,
                  color: iconColor,
