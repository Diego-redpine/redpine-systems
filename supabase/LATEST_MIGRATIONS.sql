...
CREATE POLICY "Users manage own review gate config" ON review_gate_config FOR ALL USING (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS review_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  client_id UUID REFERENCES clients(id) ON DELETE SET NULL,
  trigger_type TEXT NOT NULL DEFAULT 'manual' CHECK (trigger_type IN ('manual', 'appointment_completed', 'invoice_paid', 'bulk_campaign')),
...
CREATE POLICY "Users manage own review requests" ON review_requests FOR ALL USING (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS review_widgets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  name TEXT NOT NULL DEFAULT 'Review Widget',
  layout_type TEXT NOT NULL DEFAULT 'carousel' CHECK (layout_type IN ('list', 'grid', 'carousel', 'badge')),
...
CREATE POLICY "Users manage own review widgets" ON review_widgets FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users manage own notification prefs" ON portal_notification_preferences FOR ALL USING (auth.uid() = user_id);
...
ALTER TABLE chat_conversations ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'widget_anonymous' CHECK (source IN ('widget_anonymous', 'widget_returning', 'portal', 'sms', 'email'));
